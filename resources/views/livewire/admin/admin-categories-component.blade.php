                                ...
                                <table class="table table-striped">
                                    ...
                                    @foreach($categories as $category)
                                        <tr>
                                            ...
                                            <td>{{$category->name}}</td>
                                            <td>{{$category->slug}}</td>
                                            <td><img src="{{asset('assets/imgs/categories/'.$category->image)}}" width="100"></td>
                                            <td>{{$category->is_popular == true ? 'Yes':'No'}}</td>
                                            <td>
                                                <a href="{{route('admin.category.edit',$category->id)}}" class="text-info">Edit</a>
                                                <a href="#" class="m-3 text-danger" onclick="deleteConfirmation({{$category->id}})">Delete</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
